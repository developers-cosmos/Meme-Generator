...
class TestDataObjects(unittest.TestCase):
    ...
    def test_06_multipleinputs(self):
        ...
        _, img_encoded = cv2.imencode('.jpg', img)
        response = self.app.post(test_url, data=img_encoded.tostring(), headers=headers)
        response1 = self.app.post(test_url, data=img_encoded.tostring(), headers=headers)
        response2 = self.app.post(test_url, data=img_encoded.tostring(), headers=headers)
        response3 = self.app.post(test_url, data=img_encoded.tostring(), headers=headers)
        response4 = self.app.post(test_url, data=img_encoded.tostring(), headers=headers)
        response5 = self.app.post(test_url, data=img_encoded.tostring(), headers=headers)
        self.assertEqual(response.status_code, 200) 
